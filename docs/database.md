...
- `Conversation.model` defaults to `DEFAULT_MODEL` (`"deepseek-v4.1-flash:cloud"` by
...
- Model selections are denormalized in `users.default_model`,
  ...
  remain installed in Ollama, avoiding duplicate selector entries. Migration 047
  moves the shared built-in styles to `deepseek-v4.1-flash:cloud`, rewrites
  `users.default_model`, `room_agents.model`, and `scheduled_actions.model`,
  and registers the new model in `available_models`. It deliberately leaves
  `conversations.model` (in-flight thread history) and user-saved styles
  untouched, so those keep the model they were created with.
...
- Topics: `Topic`/`TopicAlias` graph, `TopicRelation` (044) cross-topic graph edges (`id`,`user_id`,`source_topic_id`,`target_topic_id`,`relation_type`,`confidence`,`metadata`), `Topic.centroid_embedding` vector(768) (044) for semantic topic routing, `MessageTopic` membership, `TopicAssertion`+`Evidence` claims (with vector cosine index on assertions and centroids), `TopicExclusion` suppression, `TopicContextVersion` immutable pack. The curator uses one payload-bounded call per user, fairly allocates evidence across topics from a bounded candidate scan, canonicalizes labels, builds a 3-level hierarchy, merges duplicates, and synthesizes concise grounded assertions. It promotes the resulting packs atomically.
- Assertions `uncertain|active|rejected` with `valid_from`/`valid_until`; exclusions filter before ranking.
- `TopicIngestionEvent`/`State` — idempotent queue + watermarks/lease; `ActiveContextItem` stores pinned and dynamic sources. Switching materializes the target topic's bounded baseline dynamic set inside the switch's existing context-version boundary; the first turn may rerank it.
- `TopicArchive` (042) — small index of a preserved switch boundary (`id`,`user_id`,`topic_id`,`from_topic_id`,`conversation_id`,`message_count`,`payload{topic_label,conversation_title,session_epoch}`,`short_summary`,`created_at`). Original `Message` rows remain authoritative.
- `TopicSwitchOperation` (046) — committed idempotency result keyed by `(conversation_id,idempotency_key)`, with a request fingerprint and response JSON. The locked conversation row serializes competing switches.
