...
**Phase 3 — Make the AI visibly smart (≈2 weeks)** — **COMPLETE ✅**
~~Memory ranking/budget/dedup/decay + memories-used badge~~ ✅ · ~~RAG threshold + hybrid search + citation chips~~ ✅ · ~~tool-progress streaming + human-readable tool bubbles~~ ✅ · embedding-task failure status (already present).
*Done 2026-06-11 (round 5): see above.*
*Done 2026-06-13 (round 8): `kb_min_score` relevance threshold drops barely-related chunks; hybrid retrieval fuses pgvector cosine with Postgres full-text (`ts_rank_cd`, 0.7/0.3) with a SAVEPOINT-guarded graceful fallback to semantic-only on non-Postgres/no-FTS DBs (so the shared chat transaction is never aborted); `kb_sources` filenames stamped into message metadata → citation chips under assistant replies. Tool execution streams `started`/`finished` progress chunks (with `duration_ms`) → live "running…/done in X.Xs" status on tool bubbles; tool results truncated to `tool_result_max_chars` (8000) with an explicit marker before persistence and before feeding back to the model. NB: the scheduled cloud routine for this package fired but produced no branch/PR, so it was implemented directly; 412 backend + 171 frontend tests pass.*
