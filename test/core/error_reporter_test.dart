...
void main() {
  ...
  test(
    'a failed report submission cannot recursively file another report',
    () async {
      var calls = 0;
      ...
      reporter = ErrorReporter(
        ...
        submit: (_) async {
          ...
          await reporter.report(
            StateError('report transport failed'),
            StackTrace.current,
          );
        },
      );
  ...
  test('network report title uses the structured backend detail', () async {
    final payloads = <Map<String, dynamic>>[];
    ...
    final error = DioException(
      ...
      response: Response(
        ...
        statusCode: 502,
        data: {
          'detail': 'STT service unavailable: libcublas could not be loaded',
        },
      ),
    );

    await reporter.reportNetworkError(error);

    expect(
      payloads.single['title'],
      'Backend error (502): '
      'STT service unavailable: libcublas could not be loaded',
    );
  });
}
