...
- [ ] **[Frontend] Decouple ModelProvider ↔ ChatProvider with ProxyProvider** ↩︎ — still open from `improvement.md`: `ChatPage` wires `ChatProvider(selectedModelId: () => modelProvider.selectedModelId)` through a manual callback captured at build time (`chat_page.dart:49–56`). Use `ChangeNotifierProxyProvider` so model changes flow through the tree without the closure indirection. *(S)*
- [ ] **[Frontend] Provide MemoryProvider / KnowledgeBaseProvider via the tree** ↩︎ — both pages still instantiate providers inside `State` (`memory_page.dart:22`, `knowledge_base_page.dart:24`), bypassing the tree. The KB page's polling timer can outlive navigation. Provide them at the route level (natural once routing lands — do together with the routing task). *(S)*
...
- [ ] **[Frontend] Extract and de-duplicate the theme** — `main.dart:33–82` defines light and dark themes as two near-identical 25-line blocks differing only in `brightness`. Extract `core/theme.dart` with a single `buildTheme(Brightness)` and put *all* shared component theming there (card shape, input borders, app bar) so pages stop re-styling locally. Prerequisite for the identity task below. *(XS)*
...
- [ ] **[Backend] Startup config validation** — `SECRET_KEY` is required but nothing validates it isn't a placeholder in prod; `MICROAPPS_*`, `FIREBASE_*`, and ngrok settings fail at first use rather than at boot. Add a lifespan check that logs a clear table of enabled/disabled features and hard-fails on placeholder secrets when `debug=False`. *(S)*
