...
- [x] **[Frontend] Split the two remaining god files** ↩︎ *(chat_provider → ConversationListController (list/pin/undo-delete, notifications forwarded); chat_page → ChatAppBar + PanelResizeHandle extracted, drop-validation now shares FilePickerHelper.validate with the picker path — the fork is gone)*:
...
- [x] **[Frontend] Accessibility pass** ↩︎ *(1: all 13 remaining tooltip-less IconButtons fixed; 2: semanticLabel on attachment images, image viewer, room avatars; 4: static empty-state text raised from 0.5–0.85-alpha onSurfaceVariant to full — remaining low-alpha uses are hover-dimmed icons/disabled states. Item 3 (TalkBack/Orca run) deferred to the end-of-backlog E2E pass per Jorge.)* — still essentially unstarted: exactly **1** `Semantics`/`semanticLabel` usage in the entire `lib/` tree. The June review's message-action tooltips shipped (tooltips give labels for free), but: icon-only buttons elsewhere (composer, room actions, sidebar), missing focus outlines on text fields, and the `onSurfaceVariant`-at-low-alpha contrast issue remain. Concrete pass: (1) every `IconButton` gets a `tooltip:`; (2) images/avatars get `semanticLabel`; (3) run the app with TalkBack/Orca once and fix what's unreadable; (4) verify AA contrast on secondary text in both themes. *(M)*
...
- [x] **[Frontend] Conversation search on mobile** ↩︎ *(search icon in the narrow-layout app bar opens a bottom-sheet reusing SearchWidget/SearchResultsWidget; result tap loads + dismisses, dismiss clears stale search state)* — search is still desktop-sidebar-only; mobile gets no entry point. Add a search icon to the mobile app bar routing to the same `SearchProvider` flow. *(S)*
- [x] **[Frontend] Micro-app panel discoverability** *(panel controller keeps app state across close; new canReopen/reopen + app-bar chip (wide) / icon (narrow) restore it without re-running the tool)* — the panel opens automatically from tool results or via the composer button, but on narrow screens it takes over full-screen (`chat_page.dart:501–507`) with `showCloseAsBack` as the only affordance, and once closed there's no way to re-open it without re-triggering the tool. Track "panel was open for this conversation" and offer a re-open chip in the app bar. *(S)*
- [x] **[Frontend] Loading-state consistency** ↩︎ *(core/widgets/skeleton.dart: SkeletonPulse/SkeletonListTile/SkeletonList; first-load skeletons in conversations sidebar, rooms list, room open, memories, KB docs, notifications — spinners kept only for in-flight actions; sidebar refresh no longer blanks the loaded list)* — partially addressed since June but still mixed: bare `CircularProgressIndicator` centers in some pages, text in others, nothing during room open. Standardize: skeleton rows for list pages (conversations, rooms, memories, KB docs), inline spinners for actions. Build 1–2 shared skeleton widgets first. *(M)*
...
- [x] **[Frontend] Room message visual parity check** *(verified: agent bubbles already share MessageContent→MarkdownWidget, ThinkingContent, Copy/Speak, metadata with 1:1 chat; one divergence found & fixed — human room messages rendered plain SelectableText, now use the same MessageContent markdown path as 1:1 user bubbles)* — the room redesign introduced `RoomMessageBubble` (490 lines) separate from `ChatMessageWidget`; verify markdown rendering there goes through the same memoized `MarkdownWidget` path and that agent thinking blocks, code blocks, and copy actions behave identically to 1:1 chat. Divergence here will read as jank to users who use both. *(S, mostly verification)*
...
- [x] **[Backend] Rate limiting on expensive endpoints** ↩︎ *(in-process token-bucket dependency in core/rate_limit.py on chat stream/regenerate/edit, tts speak(+stream), stt transcribe; RATE_LIMIT_ENABLED=false by default, per-scope per-minute settings, 429 + Retry-After; unit-tested)* — still open and still worth it pre-multi-user: `slowapi` (or a 30-line token-bucket dependency) on `/chat/**/chat`, `/tts/*`, `/stt/*`, keyed by user. Config-driven limits so dev stays unlimited. *(S–M)*
