...
- Ollama thinking effort & GLM suppression: Ollama supports `think` values
  `off`, `low`, `medium`, `high`. For GLM models (`glm-5.2`, `glm-5.3`, etc.),
  sending `think: False` breaks Ollama's internal thinking parser and leaks raw
  `<think>...</think>` tokens into `content`. `OllamaProvider._resolve_think_kwarg`
  maps `off` on GLM models to `think="low"` with stream suppression. Streaming
  and persistence layers (`agent_turn.py`, `ChatMessageOut`) provide defense-in-depth
  tag stripping so reasoning traces never leak into message bodies.
